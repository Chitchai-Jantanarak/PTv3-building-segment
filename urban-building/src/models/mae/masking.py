...
class BlockMasking:
    ...
    def __call__(
        ...
        for b in unique_batches:
            ...
            # Keep enough visible blocks so at least min_visible points stay visible.
            # This prevents handing PTv3 a sample with 0 visible points, which would
            # produce a gap in batch IDs and corrupt spconv's offset-based indexing.
            min_vis_blocks = max(1, n_blocks - int(n_blocks * self.ratio))
            if self.min_visible > 0 and n_sample > 0:
                # Estimate blocks needed to cover min_visible points
